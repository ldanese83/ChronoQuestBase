...
<li class="nav-item dropdown no-arrow mx-1">
    <a class="nav-link dropdown-toggle" href="#" id="studentAlertsDropdown" role="button"
       data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        <i class="fas fa-bell fa-fw"></i>
        <span class="badge bg-danger badge-counter"><?= $alertsCount > 0 ? $alertsCount : '' ?></span>
    </a>
    <div class="dropdown-list dropdown-menu dropdown-menu-end shadow animated--grow-in" aria-labelledby="studentAlertsDropdown">
        <h6 class="dropdown-header"><?= $translator->translate('alerts.dropdown.title') ?></h6>
        <button type="button" class="dropdown-item text-center small text-gray-500 border-0 bg-white" id="studentMarkAllAlertsReadButton">
        ...
        <?php else: ?>
            <?php foreach ($alerts as $alert): ?>
                <a class="dropdown-item d-flex align-items-center" href="/studenti/alerts/<?= (int) $alert['id'] ?>/open" data-student-alert-open-url="/studenti/alerts/<?= (int) $alert['id'] ?>/open">
                    <div class="mr-3">
        ...
        <a class="dropdown-item text-center small text-gray-500" href="/studenti/alerts"><?= $translator->translate('alerts.dropdown.show_all') ?></a>
    </div>
</li>
